...
public class TC047 extends SuiteOneBase {
	...
	public void SuiteOneCaseTwoTest(String Username,String Password,String State,String Profession,
			...
			String PolicyNumber,String UsernameAdmin, String PasswordAdmin,String EmailTemplateName, String SubjectLine,String PolicyNumberRenewal, String ScreenshotLink) throws Exception{
		...
		try
		{
			...
			MyProfessionalDetails.ProfessionalDetail( State, Profession,
               ...
               HomePage.SignoutButtonAdmin().click();
	 		   Add_Log.info("Clicked on Signout admin Button.");
              ...
              try{
                ...
                RenewalTesting.LooksGood().click();
                Add_Log.info("Click on Looks good Button");
				...
				Thread.sleep(3000);
				RenewalTesting.FailureCodeEntryTab().click();
				Add_Log.info("Click Failure Code Entry Tab");
				
				RenewalTesting.AmountFailureCodeEntry().sendKeys("0");
				Add_Log.info("Enter 0 in the field");	
				
				RenewalTesting.PolicyNoFailureCodeEntry().sendKeys(PolicyNumber);
				Add_Log.info("Enter policy number");
				
				RenewalTesting.PolicyNoFailureCodeEntry().sendKeys(Keys.ENTER);
				Add_Log.info("Press Enter ");
				
				Thread.sleep(2000);
				RenewalTesting.SelectpolicyFailureCodeEntry().click();
				Add_Log.info("Select the policy");
				
				RenewalTesting.RenewPolicywithCustomAmountbuttonFailureCodeEntry().click();
				Add_Log.info("click Renew Policy with Custom Amount button");
				
				
				RenewalTesting.ConfirmationOk2buttonFailureCodeEntry().click();
				Add_Log.info("First Click on Confiramtion Dailog box");
				
				RenewalTesting.ConfirmationOk1buttonFailureCodeEntry().click();
				Add_Log.info("Second Click on Confiramtion Dailog box");
				...
				for(int i =1; i<=20; i++)
				{
					...
					String SystemOptions = driver.findElement(By.xpath(".//*[@class='navbar navbar-default mx-navbar mx-name-menuBar2 hidden-xs']/ul/li[5]/ul/li"+"["+i+"]")).getText();
					System.out.println(SystemOptions);
					if (SystemOptions.equals("Email Templates"))
					{
						System.out.println("In");
						driver.findElement(By.xpath(".//*[@class='navbar navbar-default mx-navbar mx-name-menuBar2 hidden-xs']/ul/li[5]/ul/li"+"["+i+"]/a")).click();
						Thread.sleep(2000);
				...
				RenewalTesting.AdministrativeTaskTab().click();
				Add_Log.info("Click AdministrativeTask Tab");
				
				RenewalTesting.ScheduledEventSendQueuedEmails().click();
				Add_Log.info("Click Scheduled Event Button");
				
				Thread.sleep(10000);
				
				RenewalTesting.SentTab().click();
				Add_Log.info("Click Sent Tab");
				
				
				RenewalTesting.SubjectSentTab().sendKeys(SubjectLine);
				Add_Log.info("Enter Subject line");
				
				RenewalTesting.PolicyNoSentTab().sendKeys(PolicyNumberRenewal);
				Add_Log.info("Enter Policy number");
				...
				Add_Log.info("Template Subject Name :"+TemplateSubjectName);
				
				if((TemplateSubjectName.equals("Notice of Payment Failure")))
				{
					SuiteUtility.WriteResultUtility(FilePath, TestCaseName, "EmailTemplateName(Captured)", DataSet+1, TemplateSubjectName);
				System.out.println("in");
				}
